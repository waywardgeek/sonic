...
static void sonicDestroySpectrogram(sonicSpectrogram spectrogram) {
  if (spectrogram != NULL) {
    if (spectrogram->spectrums != NULL) {
      int i;
      for (i = 0; i < spectrogram->numSpectrums; i++) {
...
static sonicSpectrogram sonicCreateSpectrogram(int sampleRate) {
  sonicSpectrogram spectrogram =
...
static sonicBitmap sonicCreateBitmap(unsigned char* data, int numRows,
                                     int numCols) {
